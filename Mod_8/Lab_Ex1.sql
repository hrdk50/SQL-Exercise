...
AS SELECT p.ProductID, p.Name, p.ProductNumber AS 'Product Number', COALESCE(p.Color, 'N/A') AS 'Color',
...
AS SELECT p.ProductID AS 'Product ID', p.Name, pm.ProductModelID AS 'Product Model ID', pm.Name as 'Product Model'
FROM Production.Product AS p
